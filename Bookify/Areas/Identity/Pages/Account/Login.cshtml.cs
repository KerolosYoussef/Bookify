        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var roles = _signInManager.UserManager.GetRolesAsync(user).Result.ToArray();
                    // check if users are admin then redirect to admin dashboard or home page if they are not
                    return LocalRedirect(roles.Contains(UserRoleTypes.Admin) ? Url.Content("~/Admin/Dashboard") : returnUrl);
                }
